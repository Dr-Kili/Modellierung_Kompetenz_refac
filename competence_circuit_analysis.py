...
# Standard Modell implementieren in dimensionless Form
# Gleichungen (S10)-(S11) von SUel
def default_params():
    params = {
...
def suel_params(scale_factor=1.0):
    ...
    params = {
        'ak': 0.00875,  # ComK basal
        'bk': 7.5,      # ComK Sättigungsrate
        'bs': 0.06,     # ComS Expressionsrate
        'k0': 5000,
        'k1': 833, 
        'n': 2,         # Hill für ComK
        'p': 5,         # Hill für ComS Repression
        # Supplementory Tabelle
        'GammaK': 25000,  # ComK half maximum
        'GammaS': 20,     # ComS halb maximum
        'lambdaK': 1e-4,  # Degrationsterme
        'lambdaS': 1e-4,  
        'deltaK': 0.001 * scale_factor,  
        'deltaS': 0.001 * scale_factor   
    }
    return params
...
def find_fixed_points(params, K_range=np.linspace(0.001, 2, 100), S_range=np.linspace(0.001, 2, 100)):
   ...
   # Verschiedene Startwerte für K und S
...
def classify_fixed_point(K, S, params):
    ...
    # Determinante und Spur der Jacobi-Matrix
    det = np.linalg.det(J)
    trace = np.trace(J)
...
# Plot Phasendiagramme => redundant, könnte ich weiter refaktorisieren
