"""
Hauptskript 
Auswahl und Ausführung der Analysen für das Kompetenznetywerk von B. subtilis
"""
...
from visualization import plot_phase_diagram, plot_time_series, plot_parameter_correlations, analyze_parameter_effects
...
def analyze_standard_parameters(results_dir, params):
    """
    Analyse Standardparameter-Set des Modells.
    ...
    if is_exc:
        ...
        print("The standard system is NOT EXCITABLE.")
    ...
    if fixed_points:
        ...
        fig_phase, ax_phase = plot_phase_diagram(
            ...
            "Phase Diagram mit Standard Parametern",
            fixed_points, 
    ...
    fig_time, ax_time, comp_periods = plot_time_series(t, K, S, 
                                           title="Time Series Standard Parameter")
...
def run_suel_parameter_search(results_dir, params):
    ...
    if excitable_configs:
        ...
        # Neu: Führe stochastische Analyse für repräsentative Konfigurationen durch
        if params["run_stochastic_analysis"]:
            print("\n=== RUNNING STOCHASTIC ANALYSIS FOR REPRESENTATIVE CONFIGURATIONS ===")
            
            # Erstelle eine Liste der Parameter-Sets für die stochastische Analyse
            stochastic_param_sets = []
            param_names = []
            
            # Lade Standardparameter als Referenz
            std_params = comp_model.default_params()
            ...
            # Füge repräsentative Konfigurationen hinzu
            for i, config in enumerate(representative_configs):
                stochastic_param_sets.append(config['params'])
                param_names.append(config['name'])
            
            # Noise-Parameter für stochastische Simulation
            noise_params = {
            ...
            # Erstelle Unterverzeichnis für stochastische Analyse
            stochastic_dir = os.path.join(suel_dir, 'stochastic_analysis')
            os.makedirs(stochastic_dir, exist_ok=True)
            
            # Führe stochastische Analyse durch
            stochastic_results = analyze_stochastic_competence(
                ...
                initial_conditions=[params.get("initial_comk", 0.01), params.get("initial_coms", 0.2)],
                initial_coms_boost=params.get("initial_coms_boost", False)
            )
            
            # Neu: Führe Parametereffekte-Analyse durch
            print("\n=== RUNNING PARAMETER EFFECTS ANALYSIS ===")
            analyze_parameter_effects(representative_configs, stochastic_results, suel_dir)
...
def run_stochastic_analysis(results_dir, params, excitable_configs=None):
    """
    Optimiertes Rauschen - ausbaufähig.... wenn ich noch Zeit habe...
